...
class FastApiLoggingManager:
    ...
    @router.get("/api/logging", summary="Get logging levels for all configured loggers", require_admin=True)
    ...
    def set(self, logger_name, level, trans: ProvidesUserContext = DependsOnTrans):
        ...
        trans.app.queue_worker.send_control_task(
            "set_logging_levels", kwargs={"name": logger_name, "level": level}, get_response=True
        )
        return get_log_levels(logger_name)
