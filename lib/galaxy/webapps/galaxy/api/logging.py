...
from galaxy.util.logging.methods import get_log_levels
...
class FastApiLoggingManager:
    ...
    def set(self, logger_name, level, trans: ProvidesUserContext = DependsOnTrans):
        ...
        trans.app.queue_worker.send_control_task("set_log_level", kwargs={"name":logger_name, "level":level}, get_response=True)
        return get_log_levels(logger_name)
